...
public class MainActivity extends AppCompatActivity {
    ...
    public void testGson(View view) {
        ...
        GsonItem item = getGson().fromJson(CAT_JSON,GsonItem.class);

        String url = item.url;
        //Log.d("MainActivity",item.url);
    ...
    public void testHttpURLConnectionSync(View view) {
        ...
        new Thread(){
            ...
            public void run(){
                ...
                Log.d("MainActivity",s);//不断取到下一个json?
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        tvOut.setText(s);//上述的s需变为final
                        //Log.d("MainActivity","success");
                    }
                });
            }
        }.start();//这里的start是启动Thread

    }

    public void testRetrofitSync(View view) throws Exception {//和上一个同理，在非主线程完成网络请求，但使用retrofit
    ...
    public void testUpdateUI(View view) {
        ...
        new Thread() {
            @Override public void run() {
                ...
                runOnUiThread(new Runnable() {//回到主线程执行ui的修改
                    @Override
                    public void run() {
                        tvOut.setText(s);
                    }
